...
public class SpadeController : MonoBehaviour
{
    ...
    public void OnCollisionEnter(Collision collision)
    {
        ...
        if (isGrabed && collision.contacts[0].thisCollider == spadeTipCollider && (collision.gameObject.CompareTag("Sand")|| collision.gameObject.CompareTag("ChestSand")) )
        {
            ...
            {
                //collision.gameObject.transform.GetComponent<SandController>().BreakSand();
                collision.gameObject.SetActive(false);
            }

            if(collision.gameObject.CompareTag("ChestSand"))
            {
